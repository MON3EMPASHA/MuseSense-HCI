...
download_model("https://storage.googleapis.com/mediapipe-models/pose_landmarker/pose_landmarker_lite/float16/latest/pose_landmarker_lite.task", "pose_landmarker.task")
download_model("https://storage.googleapis.com/mediapipe-models/hand_landmarker/hand_landmarker/float16/latest/hand_landmarker.task", "hand_landmarker.task")
download_model("https://storage.googleapis.com/mediapipe-models/face_landmarker/face_landmarker/float16/latest/face_landmarker.task", "face_landmarker.task")

pose_landmarker = mp_vision.PoseLandmarker.create_from_options(mp_vision.PoseLandmarkerOptions(
    base_options=mp_python.BaseOptions(model_asset_path="pose_landmarker.task"),
    running_mode=mp_vision.RunningMode.IMAGE, min_pose_detection_confidence=0.65))
hand_landmarker = mp_vision.HandLandmarker.create_from_options(mp_vision.HandLandmarkerOptions(
    base_options=mp_python.BaseOptions(model_asset_path="hand_landmarker.task"),
    running_mode=mp_vision.RunningMode.IMAGE, num_hands=2, min_hand_detection_confidence=0.65))
face_landmarker = mp_vision.FaceLandmarker.create_from_options(mp_vision.FaceLandmarkerOptions(
    base_options=mp_python.BaseOptions(model_asset_path="face_landmarker.task"),
    running_mode=mp_vision.RunningMode.IMAGE, min_face_detection_confidence=0.65))
            ...
            if known_matrix is not None and len(known_matrix_idxs) > 0:
                ...
                if best_sim >= RECOGNITION_THRESHOLD:
                    ...
                    if len(known_samples[person_idx]) < MAX_SAMPLES_PER_PERSON:
                        ...
                        print(f"[FACE] Updated {match_name} — now {len(known_samples[person_idx])} samples, sim={best_sim:.4f}")
...
def load_camera_config(path):
    default_cfg = {"camera_mode": "local_webcam", "phone_ip_url": "http://192.168.1.18:8080/video", "local_webcam_index": 0}
    try:
...
camera_disabled = bool(camera_disabled_cfg) or camera_disabled_env in ("1", "true", "yes", "on")

primary_source = phone_ip_url if camera_mode == "phone_ip" else local_webcam_index
primary_source_name = f"IP camera ({primary_source})" if camera_mode == "phone_ip" else f"local webcam (index {primary_source})"

camera_source_env = os.getenv("CAMERA_SOURCE", "").strip()
if camera_source_env:
    primary_source = int(camera_source_env) if camera_source_env.isdigit() else camera_source_env
...
def backend_to_name(b):
    if b == getattr(cv2, "CAP_DSHOW", object()): return "CAP_DSHOW"
    if b == getattr(cv2, "CAP_MSMF", object()):  return "CAP_MSMF"
    return "default"

def choose_backend_candidates(source, preferred_backend=None):
    if not isinstance(source, int): return [None]
    seen, out = set(), []
    for c in [preferred_backend, getattr(cv2, "CAP_MSMF", None), getattr(cv2, "CAP_DSHOW", None), None]:
        if c not in seen:
...
def reconnect_camera(reason="generic"):
    ...
    if reason == "blank" and isinstance(primary_source, int):
        if preferred_webcam_backend == getattr(cv2, "CAP_DSHOW", object()) and hasattr(cv2, "CAP_MSMF"):
            ...
            preferred_webcam_backend = cv2.CAP_DSHOW
    cap = open_capture_with_retry(primary_source, primary_source_name, retries=2, delay_sec=0.5, preferred_backend=preferred_webcam_backend)
    ...
    if cap is not None: frames_since_reconnect = 0
...
while True:
    ...
    if camera_unavailable_mode:
        ...
        if not sent_face_lost_when_camera_unavailable:
            with conn_lock:
                if conn:
                    try: conn.send("face:lost".encode('utf-8'))
    ...
    if cap is None or not cap.isOpened():
        if not reconnect_camera():
            camera_unavailable_mode = True; continue
    ...
    if not ret or frame is None or frame.size == 0:
        ...
        read_fail_streak = 0; reconnect_cycle_count = 0
    ...
    try:
        ...
        if frame_is_effectively_blank(f_frame):
            ...
            blank_frame_streak = 0; reconnect_cycle_count = 0
        ...
        if worker_name:
            ...
            if not face_lost_sent and last_face_seen_frame >= 0 and (frame_count - last_face_seen_frame) >= 30:
                recent_names.clear()
                face_lost_sent = True
                msg = "face:lost"
        ...
        if frame_count % FACE_RECOGNITION_EVERY == 0:
            print(f"[HOLISTIC] face={len(face_result_mp.face_landmarks)>0}, "
        ...
        for face_lms in face_result_mp.face_landmarks:
            for lm in face_lms:
                cv2.circle(annotated, (int(lm.x * w), int(lm.y * h)), 1, (0, 200, 255), -1)
        for pose_lms in pose_result.pose_landmarks:
            draw_landmarks_cv(annotated, pose_lms, POSE_CONNECTIONS, color=(0, 255, 0))
        for hand_lms in hand_result.hand_landmarks:
            draw_landmarks_cv(annotated, hand_lms, HAND_CONNECTIONS, color=(255, 0, 128))
            ...
            with conn_lock:
                if conn:
                    try: conn.send(msg.encode('utf-8'))
                        ...
                        print(f"[SOCKET] Send failed: {e}"); conn = None
                else:
